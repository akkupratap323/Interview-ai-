...
const createResponse = async (payload: any) => {
  ...
  try {
    ...
    console.log("📝 ResponseService.createResponse - Creating response in database...");
    ...
    console.log("✅ ResponseService.createResponse - Success! Created response with ID:", data.id);
    return data.id;
  } catch (error) {
...
const getAllResponses = async (interviewId: string) => {
  try {
    console.log("📊 ResponseService.getAllResponses - Fetching for interview:", interviewId);
    ...
    console.log("📊 ResponseService.getAllResponses - Found total responses:", allResponses.length);
    ...
    console.log("📊 ResponseService.getAllResponses - Ended responses:", endedResponses.length);
    
    const responsesWithDetails = endedResponses.filter(r => r.details !== null && r.details !== undefined);
    console.log("📊 ResponseService.getAllResponses - With details:", responsesWithDetails.length);

    // For debugging: Show ALL responses for this interview to see what's in the database
    console.log("📊 ResponseService.getAllResponses - Showing ALL responses for debugging");
    
    // Temporarily return ALL responses to debug what's happening
    const finalResponses = allResponses;
    
    console.log("📊 ResponseService.getAllResponses - Final responses (all for debugging):", finalResponses.length);
...
const getResponseByCallId = async (id: string) => {
  try {
    console.log("🔍 ResponseService.getResponseByCallId - Looking for call_id:", id);
    
    const response = await db.response.findUnique({
      where: { call_id: id }
    });

    console.log("🔍 ResponseService.getResponseByCallId - Query result:", response ? "FOUND" : "NULL");
    
    if (response) {
      console.log("🔍 ResponseService.getResponseByCallId - Found response with ID:", response.id);
      console.log("🔍 ResponseService.getResponseByCallId - Response details:", {
