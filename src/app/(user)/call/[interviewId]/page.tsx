...
function InterviewInterface({ params }: Props) {
  ...
  useEffect(() => {
    const fetchinterview = async () => {
      console.log(
        "🎬 Interview Page - Starting fetch for interviewId:",
        params.interviewId,
      );
      ...
      try {
        ...
        console.log(
          "🎬 Interview Page - Method 1: Trying context getInterviewById",
        );
        ...
        const directResponse = await InterviewService.getInterviewById(
          params.interviewId,
        );
        console.log(
          "🎬 Interview Page - Direct service response:",
          directResponse,
        );

        if (directResponse) {
          console.log(
            "✅ Interview Page - Found via direct service:",
            directResponse.name,
          );
        ...
        const apiResponse = await fetch(
          `/api/test-interview-by-id?id=${encodeURIComponent(params.interviewId)}`,
        );
        ...
        if (apiData.success && apiData.interview) {
          console.log(
            "✅ Interview Page - Found via API:",
            apiData.interview.name,
          );
        ...
        console.log(
          "❌ Interview Page - All methods failed, showing invalid URL",
        );
