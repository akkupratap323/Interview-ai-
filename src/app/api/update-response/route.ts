...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      console.log("📝 Response lookup result:", existingResponse ? "FOUND" : "NOT FOUND");
    } catch (lookupError) {
    ...
    if (!existingResponse) {
      ...
      console.log("📝 This usually means the response was never created during interview start");
      console.log("📝 We cannot safely create it now due to unique constraints");
      
      return NextResponse.json({
        success: false,
        error: "Response record not found",
        message: "The response was never created during interview start. This suggests an issue with the interview initialization process.",
        callId: callId,
