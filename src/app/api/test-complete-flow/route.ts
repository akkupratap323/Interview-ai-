...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log("🧪 Test - Starting complete flow test for interview:", interviewId);
    ...
    if (!responseId) {
      return NextResponse.json({ error: "Failed to create test response" }, { status: 500 });
    }
    ...
    await ResponseService.saveResponse({
      ...
      analytics: {
        overallScore: 8,
        skillsAssessment: [
          { skill: "Communication", score: 8, feedback: "Good communication skills" },
          { skill: "Technical Knowledge", score: 7, feedback: "Solid technical foundation" }
        ],
        questionAnalysis: [
          { question: "Tell me about yourself", response: "Good response", score: 8 }
        ],
