...
export async function POST(req: Request, res: Response) {
  ...
  try {
    ...
    if (body.organizationName) {
      const interviewNameSlug = payload.name
        ?.toLowerCase()
        ...
        return NextResponse.json(
          {
            ...
            details:
              "The organization may need to be created first. Please try signing out and back in.",
          },
          { status: 400 },
        );
      }
        ...
        return NextResponse.json(
          {
            ...
            details:
              "The user may need to be created first. Please try signing out and back in.",
          },
          { status: 401 },
        );
      }
        ...
        return NextResponse.json(
          {
            ...
            details:
              "An interview with a similar name already exists. Please try a different name or try again.",
          },
          { status: 409 },
        );
      }
