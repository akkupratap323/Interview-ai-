...
export async function GET() {
  try {
    ...
    try {
      ...
      return NextResponse.json({ error: "Database connection failed", details: dbError instanceof Error ? dbError.message : String(dbError) });
    }
    ...
    const userInterviews = userId ? await db.interview.findMany({
      where: { user_id: userId },
      orderBy: { createdAt: 'desc' }
    }) : [];
    
    // Get interviews for this organization
    const orgInterviews = orgId ? await db.interview.findMany({
